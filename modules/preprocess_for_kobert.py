...
def delete_noise_front(x: str) -> str:
    
    while True:
        if len(x)<60 : 
            break
        elif re.findall("^[^가-힣]+?\s", x)==[] : 
            break
        else :
            x = re.sub("^[^가-힣]+?\s", "", x)
...
def preprocess(data, korean=True, space=True, condition=True):
    ...
    df["text"] = df["text"].apply(lambda x: re.sub("alpha", "α", x))
    df["text"] = df["text"].apply(lambda x: re.sub("beta", "β", x))
    df["text"] = df["text"].apply(lambda x: re.sub("theta", "θ", x))
    df["text"] = df["text"].apply(lambda x: re.sub("gamma", "γ", x))
    df["text"] = df["text"].apply(lambda x: re.sub("omega", "Ω", x))
    df["text"] = df["text"].apply(lambda x: re.sub("phi", "φ", x))
    df["text"] = df["text"].apply(lambda x: re.sub("pi", "π", x))
    ...
    df["text"] = df["text"].apply(lambda x: re.sub("_[^가-힣]{1,4}C_", "조합", x))
    df["text"] = df["text"].apply(lambda x: re.sub("_[^가-힣]{1,4}P_", "순열", x))
    ...
    if condition == True:
        df['text'] = df['text'].apply(lambda x: re.sub(r'\(단, (.+Z.+)\)', '\1', x))
...
def preprocess_noisy(data, korean=True, space=True, condition=True):
    ...
    df["text"] = df["text"].apply(lambda x: re.sub("begincases|endcases|longdiv|div", "", x))
    # Handle (1), (2), ... 
    df["text"] = df["text"].apply(lambda x: re.sub(r"\s\([0-9]\)\s", " ", x))
    ...
    df["text"] = df["text"].apply(lambda x: re.sub(r"[\(\[].+?점.*?[\)\]]", "", x))
    ...
    # Delete noise at the end
    df["text"] = df["text"].apply(lambda x: re.sub(r"(은\?|하시오|하여라).+?$", r"\1", x))
    # Delete noise at the front
    df["text"] = df["text"].apply(lambda x: delete_noise_front(x))
